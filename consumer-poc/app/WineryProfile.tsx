...
const happenings = {
  Today: [{ time: '5:30 PM', title: 'Golden-hour tasting', meta: 'Prototype event · no separate ticket shown' }],
  Tomorrow: [{ time: '6:00 PM', title: 'Live music on the patio', meta: 'Prototype event · outdoor' }],
  'This Weekend': [{ time: 'SAT · 12 PM', title: 'Weekend wine + food pairing', meta: 'Prototype event · reservations recommended' }, { time: 'SUN · 11 AM', title: 'Sunday brunch experience', meta: 'Prototype event · dining' }],
  'Next 7 Days': [{ time: 'FRI · 6 PM', title: 'Live music', meta: 'Prototype event' }, { time: 'SUN · 11 AM', title: 'Brunch', meta: 'Prototype event' }, { time: 'WED · 3 PM', title: 'Winery tour', meta: 'Prototype event' }]
}
...
export function WineryProfile({ winery }: { winery: ConsumerWinery }) {
  ...
  return <main className={`founder-modern traveler-profile theme-${w.presentation.themeKey ?? 'default'}`}>
    <div className="demo-banner">OUT IN THE VINES · TRAVELER PROFILE POC · PROTOTYPE DATA</div>
    <header className="topbar"><a className="brand" href="/"><span className="brand-mark">O</span><span className="brand-copy"><strong>Out in the Vines</strong><small>TEMECULA WINE COUNTRY</small></span></a><nav className="desktop-nav"><a href="#decide">Quick Look</a><a href="#happening">Happening</a><a href="#wine">Wine</a><a href="#feel">Experience</a><a href="#plan">Plan</a></nav><div className="specimen-switcher"><a href="/">Leoness</a><a href="/doffo">Doffo</a></div></header>
    ...
    <section id="decide" className="section traveler-shell">
      <div className="traveler-main">
        <section className="decision-card"><div className="vinny-intro"><div className="vinny-badge">V</div><div><p className="eyebrow teal">VINNY’S TAKE</p><div className="verdict">“{w.vinny.verdict}”</div></div></div><div className="decision-fit"><div><span>WORTH THE STOP FOR</span><strong>{known.join(' · ')}</strong></div><div><span>BEST FOR</span><strong>{w.presentation.vibeLabels.slice(0,3).join(' · ')}</strong></div><div><span>PLAN AROUND</span><strong>{w.planning.reservations ?? 'Reservation details still being verified'}</strong></div></div></section>

        <section className="glance-rich traveler-glance"><div className="section-head"><div><p className="eyebrow gold">AT A GLANCE</p><h2>The 30-second decision</h2></div></div><div className="glance-grid"><Fact label="Tasting" value={isDoffo ? 'Relaxed, personality-led' : 'Hosted, unhurried'} /><Fact label="Typical tasting" value="$30–$45 · illustrative" /><Fact label="Reservations" value={w.planning.reservations}/><Fact label="Food" value={food}/><Fact label="Time to allow" value={w.planning.visitLength}/><Fact label="Groups" value={w.planning.groups}/><Fact label="Best known for" value={known.slice(0,2).join(' · ')}/><Fact label="Welcome" value="Review in progress"/></div></section>

        <section id="happening" className="happening-block traveler-section"><div className="section-head"><div><p className="eyebrow teal">WHAT’S HAPPENING</p><h2>What’s on when you’re going?</h2></div></div><div className="date-tabs">{(Object.keys(happenings) as (keyof typeof happenings)[]).map(tab => <button key={tab} className={happeningTab === tab ? 'active' : ''} onClick={() => setHappeningTab(tab)}>{tab}</button>)}</div><div className="event-list">{happenings[happeningTab].map(event => <article key={event.time+event.title}><span>{event.time}</span><div><strong>{event.title}</strong><small>{event.meta}</small></div><button>Details →</button></article>)}</div><button className="text-link">View full event calendar →</button></section>

        <section id="wine" className="wine-portfolio traveler-section"><div className="section-head"><div><p className="eyebrow gold">WINE AT A GLANCE</p><h2>What are you actually drinking?</h2></div></div><p className="portfolio-note">{w.domains.wine?.summary ?? 'Portfolio character is still being verified.'}</p><div className="portfolio-bars">{portfolio.map(([label,value]) => <div className="portfolio-row" key={label as string}><span>{label}</span><div><i style={{width:`${value}%`}} /></div></div>)}</div><div className="tags">{w.domains.wine?.highlights?.map(item => <span key={item} className="tag">{item}</span>)}</div><button className="text-link">Explore full wine portfolio →</button></section>
        ...
        <section id="feel" className="traveler-section feel-block"><div className="section-head"><div><p className="eyebrow plum">WHAT IT FEELS LIKE</p><h2>Picture your afternoon here.</h2></div></div><p className="experience-summary">{w.domains.experiences?.summary ?? w.presentation.shortDescription}</p><div className="tags">{w.domains.experiences?.highlights?.map(item => <span key={item} className="tag">{item}</span>)}</div></section>
        ...
        <section className="traveler-section welcome-travel"><p className="eyebrow teal">WILL I FEEL WELCOME HERE?</p><div className="welcome-inline"><div><h2>Review in progress</h2><p>{w.welcome.publicSummary}</p></div><button className="text-link">See Welcome Report →</button></div></section>
      ...
      <aside className="plan-rail"><div className="plan-rail-card"><strong>Plan this stop</strong><small>{w.planning.visitLength ?? 'Visit timing in review'}</small><button>♡ Save</button><button>＋ Add to My Day</button><a href={w.actions.directionsUrl ?? '#'}>Directions</a><button className="ask-vinny">Ask Vinny</button></div></aside>
    </section>
    ...
    <nav className="bottom-nav"><a href="#decide">Quick Look</a><a href="#happening">Happening</a><button className="ask-vinny">Ask Vinny</button></nav>
  </main>
}
